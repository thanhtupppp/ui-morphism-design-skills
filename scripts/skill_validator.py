#!/usr/bin/env python3
"""Core validation library for ui-morphism-design-skills."""
...
BACKTICK_PATH_RE = re.compile(
    r"`((?:references|scripts|assets|agents|skills|domains|templates|tests|evals)/[^`\s]+|(?:SKILL|README)\.md|skill\.json)`"
)
ROOT_PREFIXES = ("references/", "scripts/", "assets/", "agents/", "skills/", "domains/", "templates/", "tests/", "evals/", ".github/")
ROOT_FILES = {"SKILL.md", "README.md", "skill.json", "LICENSE", "LICENSE.md"}
DISALLOWED_DIRS = {".git", "node_modules", "__pycache__", ".pytest_cache", ".mypy_cache", ".ruff_cache", ".idea", ".vscode", "dist", "build"}
...
class Validator:
    ...
    def validate_agents(self) -> None:
        ...
        if text is None:
            return
        self.agent_fields = self.parse_openai_yaml_minimal(text, path)
        for key in ("display_name", "short_description"):
            if not self.agent_fields.get(key):
                self.error("AG010", path, f"interface.{key} is required")
        if len(self.agent_fields.get("short_description", "")) > 120:
    ...
    def validate_skill_json(self) -> None:
        ...
        for ref in parsed.get("contracts", []) if isinstance(parsed.get("contracts"), list) else []:
    ...
    def resolve_local_target(self, source: Path, target: str) -> Path:
        if target in ROOT_FILES or target.startswith(ROOT_PREFIXES):
    ...
    def validate_links(self, source: Path, text: str) -> None:
        ...
        for raw_target in sorted(candidates):
            if not raw_target or raw_target.startswith(("http://", "https://", "mailto:", "#", "data:")):
                continue
            target = raw_target.split("#", 1)[0].split("?", 1)[0]
            if not target:
                continue
            resolved = self.resolve_local_target(source, target)
            try:
    ...
    def validate_naming(self) -> None:
        ...
        for path in self.root.rglob("*"):
            ...
            if any(part in DISALLOWED_DIRS for part in rel.parts):
                continue
            if path.name in ROOT_FILES | {"openai.yaml"} or path.name.startswith("."):
                continue
    ...
    try:
        with zipfile.ZipFile(path) as archive:
            ...
            for info in infos:
                ...
                if any(part in DISALLOWED_DIRS for part in pp.parts):
                    add("ERROR", "ZP016", info.filename, "development/cache directory must not be packaged")
                if pp.name in DISALLOWED_FILES or pp.suffix in {".pyc", ".pyo"}:
                    add("ERROR", "ZP017", info.filename, "generated/system file must not be packaged")
            ...
            if not root_skill and len(roots) == 1:
                root_skill = next(iter(roots)) + "/SKILL.md" in files
            if not root_skill:
...
def render_json(findings: list[Finding], strict: bool) -> str:
    errors = sum(f.severity == "ERROR" for f in findings)
    warnings = sum(f.severity == "WARN" for f in findings)
    return json.dumps({"ok": errors == 0 and not (strict and warnings), "errors": errors, "warnings": warnings, "strict": strict, "findings": [asdict(f) for f in findings]}, indent=2)
...
def render_sarif(findings: list[Finding]) -> str:
    ...
    for finding in findings:
        rules.setdefault(finding.code, {"id": finding.code, "name": finding.code, "shortDescription": {"text": finding.message}})
        results.append({"ruleId": finding.code, "level": "error" if finding.severity == "ERROR" else "warning", "message": {"text": finding.message}, "locations": [{"physicalLocation": {"artifactLocation": {"uri": finding.path}}}]})
    return json.dumps({"version": "2.1.0", "$schema": "https://json.schemastore.org/sarif-2.1.0.json", "runs": [{"tool": {"driver": {"name": "ui-morphism-skill-validator", "rules": list(rules.values())}}, "results": results}]}, indent=2)
...
def run_cli(argv: list[str] | None = None) -> int:
    ...
    output = render_json(findings, args.strict) if args.format == "json" else render_sarif(findings) if args.format == "sarif" else render_text(findings, args.strict)
    if args.output:
