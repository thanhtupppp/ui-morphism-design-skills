...
def validate_threshold_config(config: dict, declared_styles: list[str]) -> list[str]:
    ...
    for style, style_config in config.get("per_style", {}).items():
        if style not in declared_styles: errors.append(f"unknown threshold style: {style}")
        if not isinstance(style_config, dict): errors.append(f"per_style.{style} must be an object"); continue
...
def main() -> int:
    ...
    ap.add_argument("--root", default="."); ap.add_argument("--intents", default="evals/intents.json"); ap.add_argument("--rules", default="evals/routing-rules.json")
    ap.add_argument("--thresholds", default="evals/thresholds.json", help="JSON quality threshold configuration")
    ap.add_argument("--predictions", help="optional JSON mapping case id to predicted primary style")
    ap.add_argument("--output", default="eval-report.json"); ap.add_argument("--skip-parity", action="store_true"); ap.add_argument("--skip-thresholds", action="store_true")
    args = ap.parse_args(); root = Path(args.root).resolve()
    intents = load(root / args.intents); rules = load(root / args.rules); predictions = load(Path(args.predictions)) if args.predictions else None
    declared_styles = list(rules.get("styles", {})); declared_style_set = set(declared_styles); results = []; fixture_errors = []
    for case in intents.get("cases", []):
        cid, expected = case.get("id"), case.get("expected_primary")
        if not cid or not case.get("intent") or expected not in declared_style_set: fixture_errors.append(f"invalid case: {cid or '<missing id>'}"); continue
    ...
    if args.skip_thresholds:
        threshold_result = {"ok": True, "skipped": True, "config_errors": [], "checks": [], "failures": [], "failure_count": 0}
    else:
        threshold_config = load(root / args.thresholds); threshold_result = evaluate_thresholds(summary, metrics, threshold_config, declared_styles)
    parity_result = {"ok": True, "skipped": True} if args.skip_parity else parity(root); summary["semantic_parity"] = bool(parity_result.get("ok")); summary["thresholds_passed"] = bool(threshold_result.get("ok")); summary["threshold_failures"] = threshold_result.get("failure_count", 0)
    ok = not fixture_errors and route_passed == total and parity_result.get("ok", False) and threshold_result.get("ok", False)
    report = {"schema_version": "1.2.0", "ok": ok, "summary": summary, "classification": metrics, "thresholds": threshold_result, "fixture_errors": fixture_errors, "routing": results, "semantic_parity": parity_result}
